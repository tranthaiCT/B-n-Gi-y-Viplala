...
//        return promotionPrice;
        // Lấy giá trị giảm giá tối đa được phép từ đối tượng "promotion"
        long discountValue = promotion.getMaximumDiscountValue();

        // Lấy giá trị giảm giá (có thể là tỷ lệ phần trăm hoặc số cố định)
        long tmp = promotion.getDiscountValue();

        // Nếu kiểu giảm giá là theo phần trăm, tính giá trị giảm dựa trên tỷ lệ phần trăm của giá
        if (promotion.getDiscountType() == Contant.DISCOUNT_PERCENT) {
            tmp = price * promotion.getDiscountValue() / 100;
        }

        // So sánh và chọn giá trị giảm thực tế (giữa giảm giá tối đa và giá trị tính toán)
        if (tmp < discountValue) {
            discountValue = tmp;
        }
        ...
        // Nếu giá sau khuyến mãi nhỏ hơn 0, đặt giá thành 0 để tránh giá âm
        if (promotionPrice < 0) {
            promotionPrice = 0;
        }

        // Trả về giá sau khi áp dụng khuyến mãi
        return promotionPrice;
    }
